...
public abstract class AbstractAutowireCapableBeanFactory extends AbstractBeanFactory
	...
	public Object applyBeanPostProcessorsAfterInitialization(Object existingBean, String beanName)
		...
		Object result = existingBean;
		for (BeanPostProcessor processor : getBeanPostProcessors()) {
			logger.info("------------execute postProcessAfterInitialization for bean:" + beanName);
	...
	protected Object createBean(String beanName, RootBeanDefinition mbd, @Nullable Object[] args)
		...
		logger.info("--------开始创建bean实例：" + beanName);
	...
	protected Object doCreateBean(final String beanName, final RootBeanDefinition mbd, final @Nullable Object[] args)
		...
		try {
			logger.info("-------属性赋值阶段:" + beanName);
			populateBean(beanName, mbd, instanceWrapper);
			logger.info("-------初始化阶段:" + beanName);
		...
		if (!Boolean.FALSE.equals(mbd.beforeInstantiationResolved)) {
			...
			if (!mbd.isSynthetic() && hasInstantiationAwareBeanPostProcessors()) {
				...
				if (targetType != null) {
					...
					logger.info("------------execute PostProcessBeforeInstantiation result:" + bean);
					if (bean != null) {
	...
	protected Object applyBeanPostProcessorsBeforeInstantiation(Class<?> beanClass, String beanName) {
		for (BeanPostProcessor bp : getBeanPostProcessors()) {
			if (bp instanceof InstantiationAwareBeanPostProcessor) {
				...
				logger.info("------------execute postProcessBeforeInstantiation for bean:" + beanName);
	...
	protected void populateBean(String beanName, RootBeanDefinition mbd, @Nullable BeanWrapper bw) {
		...
		if (!mbd.isSynthetic() && hasInstantiationAwareBeanPostProcessors()) {
			for (BeanPostProcessor bp : getBeanPostProcessors()) {
				if (bp instanceof InstantiationAwareBeanPostProcessor) {
					InstantiationAwareBeanPostProcessor ibp = (InstantiationAwareBeanPostProcessor) bp;
					logger.info("-----------execute postProcessAfterInstantiation for bean:" + beanName);
	...
	protected Object initializeBean(final String beanName, final Object bean, @Nullable RootBeanDefinition mbd) {
		if (System.getSecurityManager() != null) {
			...
			logger.info("-----------invoke aware method for bean:" + bean);
			invokeAwareMethods(beanName, bean);
		}
	...
	private void invokeAwareMethods(final String beanName, final Object bean) {
		if (bean instanceof Aware) {
			if (bean instanceof BeanNameAware) {
				logger.info("----------invoke beanNameAware for bean:" + beanName);
				((BeanNameAware) bean).setBeanName(beanName);
			}
			if (bean instanceof BeanClassLoaderAware) {
				logger.info("----------invoke BeanClassLoaderAware for bean:" + beanName);
			...
			if (bean instanceof BeanFactoryAware) {
				logger.info("----------invoke BeanFactoryAware for bean:" + beanName);
